...
from .cfuncs import (
    node_finder_use_fivebyfive_window_ver2, 
    node_finder_use_fivebyfive_window_only_hill,
    node_finder_use_fivebyfive_window_diag_only_hill,
    node_finder_for_flowdirdinf,
    _run_one_step_fivebyfive_window_ver2,
    _run_one_step_fivebyfive_window_only_hill,
    _run_one_step_fivebyfive_window_diag_only_hill,
    _run_one_step_for_flowdirdinf
    )
...
class LateralEroder(Component):
    ...
    **Additional References**
    ...
    def __init__(
        ...
        # 流路角を保存するためのフィールドを追加(2023/07/14)
        if "flow_angle" in grid.at_node:
            self._flow_angle = grid.at_node["flow_angle"]
        else:
            self._flow_angle = grid.add_zeros("flow_angle", at="node")
        ...
        if "mean_slope" in grid.at_node:
            self._mean_slope = grid.at_node["topographic__mean_slope"]
        else:
            self._mean_slope = grid.add_zeros("topographic__mean_slope", at="node")
        ...
        elif solver == "langston" and node_finder == "ffwindow_diag_only_hill":
            self.run_one_step = self.run_one_step_langston_ffwindow_diag_only_hill
        elif solver == "langston" and node_finder == "flowdirdinf":
            self.run_one_step = self.run_one_step_langston_flowdirdinf
        ...
        elif solver == "ULE" and node_finder == "ffwindow_diag_only_hill":
            self.run_one_step = self.run_one_step_ULE_ffwindow_diag_only_hill
        elif solver == "ULE" and node_finder == "flowdirdinf":
            self.run_one_step = self.run_one_step_ULE_flowdirdinf
    ...
    def get_mean_slope(self, slope: np.ndarray, proportions: np.ndarray) -> np.ndarray:
        ...
        """
        ２つの流出先の傾斜の平均を計算する関数。流出先が１つしかないノードの傾斜は、
        もう一方の流出先の傾斜に置き換えたのちに平均を計算する（つまりもとの傾斜）。

        Parameters
        ----------
        slope : np.ndarray
            ２つの流出先の傾斜, shape = (grid.number_of_nodes, 2)
        proportions : np.ndarray
            ２つの流出先の割合, shape = (grid.number_of_nodes, 2)

        Returns
        -------
        mean_slope : np.ndarray
            ２つの流出先の傾斜の平均, shape = (grid.number_of_nodes,)
            mean_slope.ndim = 1.
        """        
        

        one_flow_node = np.where(proportions[:, 1] == 0.0) # 1つの流出先しかないノード
        slope[one_flow_node, 1] = slope[one_flow_node, 0] # 1つの流出先しかないノードの傾斜を、もう一方の流出先の傾斜に置き換える
    ...
    def run_one_step_langston_ffwindow_only_hill(self, dt=1.0):
        ...
        cur = grid.at_node["curvature"]
        phd_cur = grid.at_node["phd_curvature"]
        flow_angle = grid.at_node["flow_angle"]
        ...
        for j in range(iterNum):
            ...
            if i in flowdirs:
                ...
                # between segments between five nodes
                # node_finder returns the lateral node ID and the curvature and phase delay curvature
                lat_nodes_at_i, inv_rad_curv, phd_inv_rad_curv, angle = node_finder_use_fivebyfive_window_only_hill(
                                                                                                            grid, 
                                                                                                            i, 
                                                                                                            flowdirs, 
                                                                                                            da, 
    ...
    def run_one_step_langston_ffwindow_diag_only_hill(self, dt=1.0):
        """Calculate vertical and lateral erosion for a time period 'dt'.
        ...
        nodeNum = grid.shape[0]*grid.shape[1]
        lat_nodes = np.full(shape=(nodeNum, 8), fill_value=dummy_value) # node iの上下４つのノードが入るサイズ
        i = 0
        ...
        for j in range(iterNum):
            ...
            if i in flowdirs:
                ...
                # between segments between five nodes
                # node_finder returns the lateral node ID and the curvature and phase delay curvature
                lat_nodes_at_i, inv_rad_curv, phd_inv_rad_curv, angle = node_finder_use_fivebyfive_window_diag_only_hill(
                                                                                                            grid, 
                                                                                                            i, 
                                                                                                            flowdirs, 
                                                                                                            da, 
    ...
    def run_one_step_langston_flowdirdinf(self, dt=1.0):
        """Calculate vertical and lateral erosion for a time period 'dt'.
        ...
        s = grid.at_node["flow__upstream_node_order"]
        max_slopes = grid.at_node["topographic__steepest_slope"]
        mean_slopes = grid.at_node["topographic__mean_slope"]
        flow_prop = grid.at_node["flow__receiver_proportions"]
        more_flow = np.where(flow_prop > 0.0) # ndim = 2
        mean_slopes[:] = self.get_mean_slope(slope=max_slopes, proportions=flow_prop) # ndim = 1
        max_slopes = mean_slopes
        flowdirs = grid.at_node["flow__receiver_node"]
        flow_angle = grid.at_node["flow_angle"]
        ...
        for j in range(iterNum):
            ...
            if i in flowdirs:
                ...
                # between segments between five nodes
                # node_finder returns the lateral node ID and the curvature and phase delay curvature
                lat_nodes_at_i, inv_rad_curv, phd_inv_rad_curv, angle = node_finder_for_flowdirdinf(
                                                                                            grid, 
                                                                                            i, 
                                                                                            flowdirs, 
                                                                                            da, 
            ...
            for k in range(node_num_at_i):
                ...
                if lat_node > 0:  # greater than zero now bc inactive neighbors are value -1
                    if z[lat_node] > z[i]:
                        ...
                        # node i), slumping, and instant removal.
                        if UC:
                            voldiff = critical_erosion_volume_ratio * (z[i] + dp[i] - z[flowdirs[i, more_flow[1][i]]]) * grid.dx**2 
                        # TB model: entire lat node must be eroded before lateral erosion
                        # occurs
                        if TB:
                            voldiff = critical_erosion_volume_ratio * (z[lat_node] - z[flowdirs[i, more_flow[1][i]]]) * grid.dx**2
                        # if the total volume eroded from lat_node is greater than the volume
                        # needed to be removed to make node equal elevation,
                        # then instantaneously remove this height from lat node. already has
                        # timestep in it
                        if vol_lat[lat_node] >= voldiff:
                            ...
                            self._dzlat[lat_node] = z[flowdirs[i, more_flow[1][i]]] - z[lat_node]  # -0.001
                        # after t lateral node is eroded, reset its volume eroded to
                            # zero
                            vol_lat[lat_node] = 0.0
        # combine vertical and lateral erosion.
    ...
    def run_one_step_ULE_ffwindow_only_hill(self, dt=1.0):
        ...
        cur = grid.at_node["curvature"]
        phd_cur = grid.at_node["phd_curvature"]
        flow_angle = grid.at_node["flow_angle"]  # radian
        cur[:] = 0.0
        phd_cur[:] = 0.0
        flow_angle[:] = 0.0
    ...
    def run_one_step_ULE_ffwindow_diag_only_hill(self, dt=1.0):
        """Calculate vertical and lateral erosion for a time period 'dt'.
    ...
    def run_one_step_ULE_flowdirdinf(self, dt=1.0):
        """Calculate vertical and lateral erosion for a time period 'dt'.
        ...
        max_slopes = grid.at_node["topographic__steepest_slope"]
        flow_prop = grid.at_node["flow__receiver_proportions"]
        mean_slope = grid.at_node["topographic__mean_slope"]
        mean_slope[:] = self.get_mean_slope(slope=max_slopes, proportions=flow_prop)
        max_slopes = mean_slope
        flowdirs = grid.at_node["flow__receiver_node"]
