...
@cython.boundscheck(False)
@cython.wraparound(False)
cpdef inline tuple node_finder_use_fivebyfive_window_diag_only_hill(grid, 
                                                                    DTYPE_INT_t i, 
                                                                    np.ndarray[DTYPE_INT_t, ndim=1] flowdirs, 
                                                                    np.ndarray[DTYPE_FLOAT_t, ndim=1] drain_area, 
                                                                    np.ndarray[DTYPE_INT_t, ndim=1] dwnst_nodes,
    ...
    cdef DTYPE_INT_t  n = skip_node_interval
    cdef DTYPE_INT_t  donor = find_n_upstream_nodes(i, flowdirs, drain_area, n)
    cdef DTYPE_INT_t  receiver = find_n_downstream_nodes(i, flowdirs, n)
...
@cython.boundscheck(False)
@cython.wraparound(False)
cpdef inline void _run_one_step_fivebyfive_window_diag_only_hill(
                                           grid,
                                           np.ndarray[DTYPE_INT_t, ndim=1] dwnst_nodes,
                                           np.ndarray[DTYPE_INT_t, ndim=1] flowdirs, 
                                           np.ndarray[DTYPE_INT_t, ndim=1] latero_nums, 
    ...
    for j in range(iterNum):
        ...
        if i in flowdirs:
            ...
            # between segments between five nodes
            # node_finder returns the lateral node ID and the curvature and phase delay curvature
            lat_nodes_at_i, inv_rad_curv, phd_inv_rad_curv, angle = node_finder_use_fivebyfive_window_diag_only_hill(grid, 
                                                                                                                    i, 
                                                                                                                    flowdirs, 
                                                                                                                    da, 
                                                                                                                    dwnst_nodes,
